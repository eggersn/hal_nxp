...
void ENET_Ptp1588JumpTimer(ENET_Type *base, int64_t nanosecondDiff) 
{
    ...
    if (nanosecondDiff >= 0) {
	    ...
	    handle->msTimerSecond += diffSecond;
	    ...
	    int64_t diffSecond =
		    DIV_FLOOR(nanosecondDiff + (int64_t)nanosecondTime, ENET_NANOSECOND_ONE_SECOND);

	    nanosecondTime += (int32_t)(nanosecondDiff % ENET_NANOSECOND_ONE_SECOND);

	    /* Avoid negative overflow by setting PTP time to zero in this case */
	    if (handle->msTimerSecond < INT64_MAX && (int64_t)handle->msTimerSecond < -diffSecond) {
		    handle->msTimerSecond = 0;
		    base->ATVR = 0;
	    ...
	    handle->msTimerSecond -= (uint64_t)(-diffSecond);
